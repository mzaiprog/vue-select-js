<template>
    <div ref="wrapper" class="vue-select" :class="[`direction-${direction}`]" :tabindex="isFocusing ? -1 : tabindex" @focus="focus"
        @blur="e => (searchable ? false : blur(e))" v-bind="Object.assign({}, dataAttrs, $attrs)" @keypress.enter.prevent.exact="
          () => highlightedOriginalIndex !== null && addOrRemoveOption($event, optionsWithInfo[highlightedOriginalIndex])
        " @keydown.down.prevent.exact="pointerForward" @keydown.up.prevent.exact="pointerBackward" @keydown.home.prevent.exact="pointerFirst"
        @keydown.end.prevent.exact="pointerLast" @keydown="typeAhead" :id="`vs${instance.uid}-combobox`" :role="searchable ? 'combobox' : null"
        :aria-expanded="isFocusing" aria-haspopup="listbox" :aria-owns="`vs${instance.uid}-listbox`" :aria-activedescendant="
        ...
        <div class="vue-select-header">
            <template v-if="(multiple && taggable && modelValue.length === 0) || (searchable === false && taggable === false)">
                <div class="vue-input">
                ...
                <v-tags :modelValue="optionsWithInfo" :collapse-tags="collapseTags" tabindex="-1" @click="focus">
                    <template #default="{ option }">
                        <slot name="tag" :option="option.originalOption" :remove="() => addOrRemoveOption($event, option)">
                            ...
                            <img src="./images/delete.svg" alt="delete tag" class="icon delete" @click.prevent.stop="() => addOrRemoveOption($event, option)" />
                        </slot>
                    </template>
                </v-tags>
            ...
            <template v-else>
                <v-input v-if="searchable" ref="input" v-model="searchingInputValue" :disabled="disabled" :autocomplete="autocomplete"
                    :placeholder="isFocusing ? searchPlaceholder : innerPlaceholder" @input="handleInputForInput" @change="handleChangeForInput"
                    @focus="handleFocusForInput" @blur="handleBlurForInput" @escape="blur" :autofocus="autofocus || (taggable && searchable)"
                    :tabindex="tabindex" :comboboxUid="instance.uid" />
        ...
        <div class="vue-select-input-wrapper" v-if="multiple && taggable && searchable">
            <v-input v-show="isFocusing" ref="input" v-model="searchingInputValue" :disabled="disabled" :autocomplete="autocomplete"
                :placeholder="isFocusing ? searchPlaceholder : innerPlaceholder" @input="handleInputForInput" @change="handleChangeForInput"
                @focus="handleFocusForInput" @blur="handleBlurForInput" @escape="blur" :autofocus="autofocus || (taggable && searchable)" :tabindex="tabindex"
        ...
        <v-dropdown ref="dropdown" v-model="optionsWithInfo" @click-item="addOrRemoveOption" @mouseenter="(ev, option) => pointerSet(option.originalIndex)"
            :comboboxUid="instance.uid" :maxHeight="maxHeight" :highlightedOriginalIndex="highlightedOriginalIndex">
            <template #default="{ option }">
...
<script lang="ts">
import { ref, computed, watch, provide, getCurrentInstance, nextTick, defineComponent } from 'vue'
                ...
                if (
                    Object.keys(normalizedModelValue.value).some(
                        index =>
                            ...
                            getOptionByValue(normalized.options, props.modelValue[index], {
